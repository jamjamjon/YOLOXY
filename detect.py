...
def run(
        ...
        data=ROOT / 'data/projects/coco128.yaml',  # dataset.yaml path
    ...
    # Directories
    save_dir = increment_path((Path(project) if not tracking else Path('runs/track')) / name, exist_ok=exist_ok)  # increment run
                ...
                if tracking:
                    ...
                    for t in online_targets:
                        if save_img or save_crop or view_img:  # Add bbox to image
                            label = f'{t.track_id} {t.score:.2f}'   # tracker id + score
                            annotator.box_label(t.tlbr, label, color=colors(int(t.track_id), True))

                        if save_crop:
                            save_one_box(t.tlbr, imc, file=save_dir / 'crops' / f'{p.stem}.jpg', BGR=True)

                        # '{frame},{id},{x1},{y1},{w},{h},{s},-1,-1,-1\n'
                        if save_txt:  # Write to file
                            txt_path = str(save_dir / 'labels' / p.stem)  # im.txt
                            line = (frame_id+1, t.track_id, *t.tlwh, t.score, -1, -1, -1) if save_conf else (frame_id+1, t.track_id, *t.tlwh, -1, -1, -1)  # label format
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'data/images/horse_ride.jpg', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--data', type=str, default=ROOT / 'data/projects/coco128.yaml', help='(optional) dataset.yaml path')
