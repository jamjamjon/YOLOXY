...
LOGGER = logging.getLogger("yolov5x")  # define globally (used in train.py, val.py, detect.py, etc.)

CONSOLE = Console()     # console logger
...
def print_args(args: Optional[dict] = None, show_file=True, show_fcn=False):
    ...
    # s = (f'[bold gold1]{Path(file).stem}:[/bold gold1] ' if show_file else '') + (f'{fcn}: ' if show_fcn else '')
    ...
    table.add_column("ARGUMENTS", justify="left", style="", no_wrap=True)
    table.add_column("VALUES", style="")
...
def check_amp(model):
    ...
    from models.experimental import AutoShape
    ...
    try:
        assert amp_allclose(model, im) or amp_allclose(DetectMultiBackend('N.pt', device), im)
...
def colorstr(*input):
    ...
    *args, string = input if len(input) > 1 else ('bright_cyan', 'b', input[0])  # color arguments, string
    colors = {
        ...
        'end': '\033[0m',  # misc
        'bold': '\033[1m', 'b': '\033[1m',  # bold
        'blink': '\033[5m',
        'reverse': '\033[7m',
        'underline': '\033[4m', 'u': '\033[4m', # under line
...
def scale_coords_kpts_xxyy(img1_shape, coords, img0_shape, ratio_pad=None, nk=0):
    # Rescale coords (xxyy) from img1_shape to img0_shape
    if ratio_pad is None:  # calculate from img0_shape
        gain = min(img1_shape[0] / img0_shape[0], img1_shape[1] / img0_shape[1])  # gain  = old / new
        pad = (img1_shape[1] - img0_shape[1] * gain) / 2, (img1_shape[0] - img0_shape[0] * gain) / 2  # wh padding
    else:
    ...
    coords[:, 0: nk] -= pad[0]  # x padding
    coords[:, nk: 2 * nk] -= pad[1]  # y padding
    coords[:, 0: nk] /= gain
    coords[:, nk: 2 * nk] /= gain

    clip_coords_kpts_xxyy(coords, img0_shape, nk=nk)
    return coords



def clip_coords_kpts_xxyy(boxes, shape, nk=0):
    # Clip bounding xxyy bounding boxes to image shape (height, width)
    if isinstance(boxes, torch.Tensor):  # faster individually
        boxes[:, 0: nk].clamp_(0, shape[1])  # x
        boxes[:, nk: ].clamp_(0, shape[0])  # y
    else:  # np.array (faster grouped)
        boxes[:, 0: nk] = boxes[:, 0: nk].clip(0, shape[1])  # x
        boxes[:, nk: 2 * nk] = boxes[:, nk: 2 * nk].clip(0, shape[0])  # y
...
def polygons2masks(img_size, polygons, color=1, downsample_ratio=1):
    """
    Args:
        img_size (tuple): The image size.
...
def process_mask(protos, masks_in, bboxes, shape, upsample=False):
    ...
    # print(f'maskin ---> {masks_in.shape}')
    # print(f'protos ---> {protos.shape}')
