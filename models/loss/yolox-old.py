...
class ComputeLoss:
    ...
    def __init__(self, model):
        ...
        # self.grids = [torch.zeros(1)] * self.nl     #  ([1, 1, 80, 80, 2], [1, 1, 40, 40, 2], [1, 1, 20, 20, 2])
    ...
    def __call__(self, p, targets):
        ...
        # targets cls, box, ...
        tcls, tbox, tbox_l1, tobj, candidates_masks, num_candidates = [], [], [], [], [], 0 
        ...
        for idx in range(p.shape[0]):   # batch size
            ...
            if nt == 0:     # neg sample image
                ...
                candidates_mask = p.new_zeros(self.ng).bool()
            else:   
                ...
                (
                    ...
                    matched_gt_inds,
                    num_anchor_assigned   # 每张图上的gt一共分配了多少anchor point
                 ) = self.get_assignments(t_bboxes, t_classes, p_bboxes, p_classes, p_objs)


                # TODO: put these part into get_assignments()
                torch.cuda.empty_cache() 
                num_candidates += num_anchor_assigned    # 一个batch内所有分配的anchor的数量

                # has anchor point assigned
                if num_anchor_assigned > 0:
                    ...
                    tobj_ = candidates_mask.unsqueeze(-1)
                    tbox_ = t_bboxes[matched_gt_inds]

                    # tbox_l1, do scale
                    tbox_l1_ = p.new_zeros((num_anchor_assigned, 4))
                    stride_ = self.expanded_strides[0][candidates_mask]
                    grid_ = self.xy_shifts[0][candidates_mask]
                    eps_ = 1e-8
                    tbox_l1_[:, :2] = t_bboxes[matched_gt_inds][:, :2] / stride_ - grid_
                    tbox_l1_[:, 2:4] = torch.log(t_bboxes[matched_gt_inds][:, 2:4] / stride_ + eps_)
            ...
            candidates_masks.append(candidates_mask)
        ...
        candidates_masks = torch.cat(candidates_masks, 0)
        num_candidates = max(num_candidates, 1)

        # Compute loss
        lbox += (1.0 - bbox_iou(pbox.view(-1, 4)[candidates_masks], tbox, SIoU=True).squeeze()).sum() / num_candidates  # iou(prediction, target)
        lobj += (self.BCEobj(pobj.view(-1, 1), tobj * 1.0)).sum() / num_candidates
        lcls += (self.BCEcls(pcls.view(-1, self.nc)[candidates_masks], tcls)).sum() / num_candidates
        lbox_l1 += (self.L1box(pbox0.view(-1, 4)[candidates_masks], tbox_l1)).sum() / num_candidates
    ...
    # 经过simota后，返回具体哪些anchor是正样本，那些是负样本
    @torch.no_grad()
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, t_classes, candidates_mask):
        ...
        # gt_matched_classes = t_classes[matched_gt_inds]  # update

        # 6 matching_matrix(其中除了0就是1) * iou matrix
        pred_ious_this_matching = (matching_matrix * pair_wise_ious).sum(0)[
            fg_mask_inboxes
        ]

        # return num_anchor_assigned, gt_matched_classes, pred_ious_this_matching, matched_gt_inds
        return num_anchor_assigned, pred_ious_this_matching, matched_gt_inds
